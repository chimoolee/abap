...
CLASS lcl_app IMPLEMENTATION.
  METHOD run.
    ...
    IF lt_bom_mast IS NOT INITIAL.
      ...
      IF sy-subrc = 0.
        SORT lt_comp_raw BY stlnr idnrk.
      ENDIF.
    ENDIF.

    " Map components to parents via STLNR
    LOOP AT lt_bom_mast INTO DATA(ls_mast).
      READ TABLE lt_comp_raw ASSIGNING FIELD-SYMBOL(<ls_cr>)
           WITH KEY stlnr = ls_mast-stlnr BINARY SEARCH.
      IF sy-subrc <> 0.
        CONTINUE.
      ENDIF.
    ...
    LOOP AT lt_mov INTO DATA(lv_matnr_mov).
      ...
      READ TABLE lt_mara INTO DATA(ls_mara)
           WITH KEY matnr = ls_row-matnr BINARY SEARCH.
      IF sy-subrc = 0.
        ls_row-mtart = ls_mara-mtart.
      ENDIF.
      READ TABLE lt_makt INTO DATA(ls_maktx)
           WITH KEY matnr = ls_row-matnr BINARY SEARCH.
      IF sy-subrc = 0.
    ...
    " B) Stock-only rows
    LOOP AT lt_stock INTO ls_stock.
      READ TABLE lt_mov WITH KEY table_line = ls_stock-matnr
      ...
      IF sy-subrc = 0.
        ls_row-mtart = ls_mara-mtart.
      ENDIF.
      READ TABLE lt_makt INTO ls_maktx
           WITH KEY matnr = ls_row-matnr BINARY SEARCH.
      IF sy-subrc = 0.
        ls_row-maktx = ls_maktx-maktx.
      ENDIF.
    ...
    LOOP AT lt_comp INTO ls_comp.
      ...
      ls_row-werks  = p_werks.
      READ TABLE lt_mov WITH KEY table_line = ls_row-matnr
           TRANSPORTING NO FIELDS.
      IF sy-subrc = 0.
