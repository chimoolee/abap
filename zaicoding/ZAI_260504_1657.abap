...
CLASS lcl_app IMPLEMENTATION.
  METHOD run.
    ...
    " Defaults for period: last 30 days if initial
    ...
    " A: Materials with goods movements in plant within period
    ...
    " B: Materials with current stock > 0 in plant
    ...
    " C: Materials that appear only as BOM components for FERT headers in plant
    ...
    " Build union list with precedence: Movements > Stock > BOM-only
    " 1) Movements
    ...
    " 3) BOM-only components (exclude A and B)
    ...
    IF lt_all_mat IS NOT INITIAL.
      ...
      LOOP AT lt_final INTO ls_res.
        READ TABLE lt_mara INTO DATA(ls_mara) WITH KEY matnr = ls_res-matnr BINARY SEARCH.
        IF sy-subrc = 0.
          ls_res-mtart = ls_mara-mtart.
          ls_res-meins = ls_mara-meins.
        ENDIF.
        READ TABLE lt_makt INTO DATA(ls_makt) WITH KEY matnr = ls_res-matnr BINARY SEARCH.
        IF sy-subrc = 0.
