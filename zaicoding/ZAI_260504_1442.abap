...
PARAMETERS p_date_lo TYPE sy-datum DEFAULT sy-datum.
PARAMETERS p_date_hi TYPE sy-datum DEFAULT sy-datum.
  ...
  METHOD run.
    ...
    SELECT mseg~matnr
      ...
      WHERE mseg~werks = @p_werks
        AND mseg~budat BETWEEN @p_date_lo AND @p_date_hi
    ...
    " Section 2: BOM 만 있음 (완제품의 BOM 요소 중 재고/실적 없는 자재)
    ...
    IF lt_parents IS NOT INITIAL.
      ...
      " Filter to BOM-only: not in mov list and not in stock list
      SORT lt_mov_matnr.
      SORT lt_stock_matnr.
      ...
      IF lt_bom_comp_only IS NOT INITIAL.
        DATA lt_comp_tx TYPE STANDARD TABLE OF makt WITH EMPTY KEY.
        SELECT makt~matnr, makt~maktx
          FROM makt
          WHERE makt~spras = @sy-langu
        ...
        " Map components to parent FERT via STPO and MAST
        DATA lt_map TYPE STANDARD TABLE OF stpo WITH EMPTY KEY.
