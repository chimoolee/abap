...
CLASS lcl_app IMPLEMENTATION.
  METHOD run.
    ...
    " 1) Get materials with postings in date range per plant
    SELECT DISTINCT
      s~matnr,
      s~werks
      FROM mseg AS s
    ...
    " 2) Get materials with current stock (not zero) per plant
    SELECT DISTINCT
      mard~matnr,
      mard~werks
      FROM mard
    ...
    LOOP AT lt_union_hash INTO ls_key.
      DATA(lv_has_post) = xsdbool( line_exists( lt_post_hash[ matnr = ls_key-matnr
                                                             werks = ls_key-werks ] ) ).
      DATA(lv_has_stock) = xsdbool( line_exists( lt_stock_hash[ matnr = ls_key-matnr
                                                                werks = ls_key-werks ] ) ).
